...
class LeaveCommand extends FactionsCommand{

    public function __construct(){
        parent::__construct(["leave"], "Leave your current faction", "Use: '/f leave'");
    }
    ...
    public function execute(CommandSender $sender, array $args, FactionsPM $plugin) : void{
        ...
        if($faction->isLeader($sender->getName())){
            $sender->sendMessage(TextFormat::RED . "You must '/f disband' your faction before leaving");

            return;
        }
        $moderator = $faction->isModerator($sender->getName());
        $plugin->getDatabase()->setPlayerFaction($sender->getName(), 0, "Member");
        if($moderator){
            $faction->removeModerator($sender->getName());
        }else{
            $faction->removeMember($sender->getName());
        }
        $sender->sendMessage(TextFormat::RED . "You have left " . $faction->getName());
        $faction->broadcastMessage(TextFormat::RED . $sender->getName() . " has left the faction");
    }
}